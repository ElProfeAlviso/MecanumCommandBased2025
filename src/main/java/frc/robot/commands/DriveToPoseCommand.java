...
public class DriveToPoseCommand extends Command {
  ...
  public DriveToPoseCommand(DriveTrain driveTrain, Pose2d targetPose) {
    ...
    PIDController xController = new PIDController(3, 0.001, 0.0001);
    PIDController yController = new PIDController(3, 0.001, 0.0001);
  ...
  public boolean isFinished() {
   ...
   distanceError = current.getTranslation().getDistance(targetPose.getTranslation());

    angleError = Math.abs(current.getRotation().minus(targetPose.getRotation()).
    getDegrees());

    return distanceError < 1 && angleError < 2;
